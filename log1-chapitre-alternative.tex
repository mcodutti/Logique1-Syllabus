...
\marginicon{exercice}
\begin{Emphase}{Exercice résolu : Signe d'un nombre}
...
\cadre{
\begin{pseudo}
\If{expression = une des valeurs de la liste\_1}
	\LComment instructions lorsque la valeur est dans liste\_1
\Else
	\If{expression = une des valeurs de la liste\_2}
		\LComment instructions lorsque la valeur est dans liste\_2
	\Else
		\LComment \dots
		\If{expression = une des valeurs de la liste\_n}
			\LComment instructions lorsque la valeur est dans liste\_n
		\Else
			\LComment instructions lorsque la valeur de l'expression
			\LComment ne se trouve dans aucune des listes précédentes
...
\marginicon{definition}
\begin{Emphase}{selon que (version avec conditions)}
\cadre{
\begin{pseudo}
\Switch{}
	\Case{condition\_1}
...
\begin{Emphase}[exercice]{Exercice résolu : Jour de la semaine en clair}
...
\Read jour
	\Switch{jour \K{vaut}}
		\Stmt 1 : \K{écrire} "lundi"
		\Stmt 2 : \K{écrire} "mardi"
		\Stmt 3 : \K{écrire} "mercredi"
		\Stmt 4 : \K{écrire} "jeudi"
		\Stmt 5 : \K{écrire} "vendredi"
		\Stmt 6 : \K{écrire} "samedi"
		\Stmt 7 : \K{écrire} "dimanche"
	\EndSwitch
\EndModule
\end{pseudo}
}
...
\marginicon{exercice}
\begin{Emphase}{Exercice résolu : Nombre de jours (avec énumération)}
...
\cadre{
\begin{pseudo}
\Stmt \K{énumération} Mois {JANVIER, FEVRIER, \dots, DECEMBRE}
\Module{nbJours}{}{}
	\Decl mois : Mois
	\Read mois
	\Switch{mois \K{vaut}}
		...
		\Case{AVRIL, JUIN, SEPTEMBRE, NOVEMBRE}
			\Write 30
		\Case{FEVRIER} \Comment{on ne tient pas compte ici des années bissextiles}
			\Write 28
		\Case{\K{autre}}
			\Write "Mois invalide"
	\EndSwitch
...
\marginicon{exercice}
\begin{Emphase}{Exercice : Signe d'un nombre (avec selon-que)}
...
\begin{Exercice}{Test d'intervalle}
{
