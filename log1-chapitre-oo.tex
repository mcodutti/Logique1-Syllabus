...
%============================
	...
	\code{nouveau}) est toujours là et bien là !
...
%=====================================
	...
	\begin{Emphase}{Exercice – méthode égal()}
		\marginicon{exercice}
		Écrire la méthode \code{égal()} pour la classe \code{Moment}.
...
%=======================================
	...
	\subsection{Le comportement (les méthodes)}
		...
		\begin{liste}
			...
			\item {
				...
				\code{getXXX()} et \code{getTotalXXX()}. Par
				exemple, la méthode \code{getMinute()} retourne la valeur
				de la composante «~minutes~» dans une représentation HMS tandis que la
				méthode \code{getTotalMinutes()} retourne le nombre total
				de minutes entières pour cette durée. Ex~: pour 1h23’12’’,
				\code{getMinute()} retourne 23 et
				\code{getTotalMinutes()} retourne 83. Idem avec 
				les jours, les heures	et les secondes.}
			\item {
				Les méthodes \code{getTotalXXX()} retournent le nombre
				(toujours entier) de XXX contenus dans la durée. Exemple, avec la durée
				0h23’52'’, \code{getTotalMinutes()}
				retourne 23 et pas 24 (autrement dit, il n’y a pas d’arrondi vers le
				haut).}
			\item {
				Il n’y a pas de \textit{mutateur }(\code{setXXX()}). Ce qui
				...
				précis.}
			\item {
				La méthode \code{ajouter()} ne retourne rien. En effet,
				...
				on l’aurait plutôt appelée «\code{~plus( )}~».}
			\item {
				La méthode \code{différence()}, elle, renvoie toujours une
				durée (positive).}
		\end{liste}
	...
	\subsection{La représentation de l'état (les attributs)}
		...
		introduire un champ ‘\code{jour}’ (après tout on a bien
		une méthode \code{getJour()}). 
...
%================================
	...
	%-----------------------
		...
		Par exemple, on pourra définir un \code{Etudiant} à partir
		de la notion de \code{Personne}.
...
\begin{Exercice}{Un produit}
	Reprendre la classe \code{Produit} qui a servi
...
\begin{Exercice}{Une personne}
	Créer une classe \code{Personne}, une personne étant
	...
	\begin{liste}
		\item 
			avec 3 arguments~: le nom (\code{chaine}), le prénom
			(\code{chaine}) et la date de naissance de la personne
			(\code{Date})
		\item 
			avec 2 arguments de type chaine~: le nom et le prénom de la personne; la
			date de naissance est alors initialisée à «\code{~rien~}»
	\end{liste}
	...
	\code{Date} sans paramètre.
\end{Exercice}

\begin{Exercice}{Anniversaire des personnes}
	À l'aide de la classe \code{Personne} écrite plus haut,
	écrire un module qui lit des \code{Personne} (au clavier)
...
\begin{Exercice}{Test}
	...
	fonctionnement de la classe \code{Rectangle} de l'exercice précédent. Ce module~: 
