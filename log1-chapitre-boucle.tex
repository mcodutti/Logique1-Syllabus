...
% =======================================
	...
	dossier et passer au suivant jusqu’à ce qu’il n’en reste plus~».
...
% ==============================
	...
	\subsection{«~faire~–~jusqu’à ce que~»}
		...
		\code{faire} et \code{jusqu’à ce que} 
	...
	\subsection{«~pour~»}
		...
		Dans ce type de structure, \code{début}, \code{fin} et \code{pas}
		...
		\code{pas} est facultatif, et généralement omis (dans ce cas, sa valeur 
		par défaut est 1). Ce pas est parfois négatif, dans le cas
		d’un compte à rebours, par exemple
		\code{pour n de 10 à 1 par -1}.

		Quand le \code{pas} est positif, la boucle s’arrête
		lorsque la variable dépasse la valeur de \code{fin}. Par contre, avec
		...
		qu’au cas (à éviter) où \code{début} est strictement supérieur à
		...
		de programmation~!). Idem si \code{début} est strictement inférieur à
		...
		variables de contrôle \code{début}, \code{fin} ou \code{pas}~! Il
		...
		\code{variable ← variable + pas} 
		étant automatique et implicite à chaque étape de la
		boucle. Il est aussi déconseillé d’utiliser \code{variable} à la
	...
	\subsection{Quel type de boucle choisir~?}
		...
		fois, alors que dans \code{tant que}, 
