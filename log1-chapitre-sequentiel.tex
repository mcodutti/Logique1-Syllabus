...
\chapter{Algorithmes séquentiels}
	...
	\section{Variables et types}
		...
		\subsection{Les types autorisés}
			...
			\marginicon{exercice}
			\begin{Emphase}{Exercice : Une date}
		...
		\subsection{Comment nommer correctement une variable ?}
			...
			\marginicon{exercice}
			\begin{Emphase}{Exercice : Déclarer une date}
	...
	\section{L’affectation d’une valeur à une variable}
		...
		\subsection{Affectation interne }
			...
			\cadre{
			\begin{pseudo}
			\Gets{somme}{nombre1 + nombre2}
			\Gets{denRes}{den1 * den2}
			\Gets{cpt}{cpt + 1}
			\Gets{delta}{b**2 – 4*a*c}
			\Gets{test}{a < b} \Comment{pour une variable logique}
			\end{pseudo}
			}
			...
			\item 
				...
				\textsf{x}{ $\gets$ }\textstyleCodeInsr{2 : entier}\ \ // incorrect !
			\item 
	...
	\section{Structure générale d’un algorithme}
		...
		\Module{additionnerFractions}{}{}
			...
			\Read{num1, den1, num2, den2}
			\Gets{denRes}{den1 * den2}
			\Gets{numRes}{num1*den2 + num2*den1}
			\Write{numRes, "/", denRes}
		\EndModule
		\end{pseudo}
		}
	...
	\section{Commenter un algorithme}
		...
		\cadre{
		\begin{pseudo}
		\Gets{nouveauCapital}{ancienCapital * (1 + taux / 100)}
		\end{pseudo}
		}
		...
		\cadre{
		\begin{pseudo}
		\Gets{c1}{c0 * (1 + t / 100)} \RComment calcul du nouveau capital
